...
class DataValidation:
    ...
    def validate_no_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"No of columns in Schema: {no_of_columns}")
    ...
    def numeric_column_exist(self, dataframe: pd.DataFrame) -> bool:
        try:
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold=0.05) ->bool:
        try:
            ...
            # uploading the report into the drift report filepath
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            status = self.numeric_column_exist(dataframe=train_dataframe)
            if not status:
                error_message = f"Train dataframe does not have all the numerical columns"
            status = self.numeric_column_exist(dataframe=test_dataframe)
            if not status:
                error_message = f"Test dataframe does not have all the numerical columns"
